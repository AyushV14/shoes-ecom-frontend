...
const MenuMobile = ({showCatmenu, setShowCatmenu, setMobileView, categories}) => {
  return (
                ...
                <React.Fragment key={item.id}>
                    {!!item?.subMenu ? (
                            ...
                            {showCatmenu && (
                                <ul className='bg-black/[0.05] -mx-5 mt-4 -mb-4'>
                                    {categories.map((submenu)=>{
                                        return (
                                            <Link key={submenu.id} href={`/category/${submenu.slug}`} onClick={()=>{
                                                setShowCatmenu(false);
                                                setMobileView(false)
                                            }}>
                                                <li className='py-4 px-8 border-b flex justify-between '>
                                                    {submenu.name}
                                                    <span className='opacity-50 text-sm'>{`(${submenu.products?.length})`}</span>
                                                </li>
                                            </Link>
                                        )
                                    })}
                        ...
                        <li className='py-4 px-5 border-b'>
                            <Link href={item?.url} onClick={()=>setMobileView(false)}>{item.name}</Link>
                        </li>
                    )}
                </React.Fragment>
            )
